...
@Managing
@Interceptor
internal class ManagingInterceptor(val loginService: LoginService, val permissionService: PermissionService, val siteService: SiteService) : PermittedInterceptor() {
    ...
    override fun throwingGuard(invocationContext: InvocationContext, required: Role) {
        ...
        val layout = siteService.find(Reference(uuid)).payload.role ?: throw AuthenticationFailedException()

        if (role < required && role < layout) throw AuthenticationFailedException()
    }
}